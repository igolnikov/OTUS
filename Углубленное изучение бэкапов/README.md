...
Зафиксировать состояние базы — функция pg_backup_start() переключает WAL и создаёт контрольную точку. Это можно делать только на мастере.
Поэтому нам нужно будет добаить ip мастера "-h 172.34.35.156" и реплики "--remote-host=172.34.35.154"

`sudo -u postgres pg_probackup backup   -B /mnt/data/backups   --instance=patroni_cluster_replica   -b FULL   --stream   --remote-host=172.34.35.154   --remote-user=postgres -h 172.34.35.156  -U postgres`

Что происходит при такой команде:
1. pg_probackup на сервере бэкапов подключается по SSH к реплике (154)
2. На реплике запускается агент pg_probackup, который читает файлы данных
3. Параллельно pg_probackup открывает репликационное соединение к МАСТЕРУ (156)
4. На мастере выполняется pg_backup_start() — начало "горячего" бэкапа
5. Данные копируются с реплики (файлы)
6. WAL читаются с мастера через репликационный протокол
7. На мастере выполняется pg_backup_stop() — завершение бэкапа
